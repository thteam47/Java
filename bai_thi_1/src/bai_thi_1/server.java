        ...
        try {
            ...
            do {
                ...
                if (check == 1) {
                    ...
                    ObjectOutputStream oos = new ObjectOutputStream(but);
                    oos.writeObject(list);
                    oos.flush();
                    oos.close();
                    byte[] objByte = but.toByteArray();
                    packet = new DatagramPacket(objByte, objByte.length, packet.getAddress(), packet.getPort());
                    server.send(packet);
                }
            } while (check != 4);
